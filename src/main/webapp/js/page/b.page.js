...
!function ($) {
	...
	var template =
		'<div class="pagination bPage bPageRight">' +
		 ...
		 '</ul>' +
		 '</div>';
		//document.getElementById('select_page');
	/**
	 * 构造方法
	 */
	var bPage = function(element,p){
		...
		this.p = p;
		this.pageNo = Number(p.pageNo);
		this.pageSize = Number(p.pageSize);
		this.pages = Number(p.pages);
	...
	bPage.prototype.populate = function(){
		var self = this, elem = this.$container, p = this.p, pNum = this.pageNo;
		var _class, _start, _end, _half=Math.floor(p.pageBarSize/2);
		
		var pageInfo = '当前显示第' + this.pageNo + '/' + this.pages + ' 页' + '（共'+this.total+'条记录）';
		$('#bPageInfo a',$(elem)).html(pageInfo);

		//总页数小于显示页码个数
		if(this.pages < p.pageBarSize) {
			_start = 1;
			_end = this.pages;
		//当前页码小于显示页码个数的一半
		} else if ( pNum <= _half ) {
		...
		} else if ( pNum >= (this.pages - _half) ) {
			_start = this.pages - p.pageBarSize + 1;
			_end = this.pages;
		//常规情况
		} else {
			_start = pNum - _half;
	 ...
	 */
	bPage.prototype.getParams = function(pageNo){
		var param = {}, p = this.p;
		param.pageNo = pageNo;
	 ...
	 * 设置事件
	 */
	bPage.prototype.setFunction = function(obj,pageNo){
		var self = this;
		$(obj).off(bPage.eventName).on(bPage.eventName,function(event){
			self.pageSwitch(pageNo);
		});
	};
	/**
	...
	bPage.prototype.extractPageInfo = function(data){
		...
		if(!pNum || !pSize || !tPage || !tRow){
			...
			try {
				if(pNum) this.pageNo = Number(pNum);
				if(pSize) this.pageSize = Number(pSize);
				if(tPage) this.pages = Number(tPage);
				if(tRow) this.total = Number(tRow);			
			} catch (e) {
				done = false;
				console.error(errorMsg);
			}
	 ...
	 */
	bPage.prototype.pageSwitch = function(pageNo){
		var self = this, p = this.p;
		if(typeof(pageNumer)==undefined && typeof(pageNo)!='number') pageNo = self.pageNo;
		if(pageNo == -1) return;
		if(pageNo > self.pages) pageNo = self.pages;
		//异步刷新页面模式
		if(p.asyncLoad){
			self.pageNo = pageNo;
			var param = self.getParams(pageNo);
			var async = true;
			...
			if(p.serverSidePage){
				...
				$.ajax({
					...
					dataType : 'json',
					success : function(returnData){
						self.pageNo = returnData.pageNo;
						self.pageSize = returnData.pageSize;
						self.pages = returnData.pages;
			...
			//直接跳转模式
			window.location.href = self.setUrl(pageNo);
		}
	};
	/**
	 * 设置具体页码跳转的URL
